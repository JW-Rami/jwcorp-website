...
$(window).resize();
    ...
    var mq2 = window.matchMedia( "(max-width: 390px)" );
    if (mq.matches) {
  ...
  // Span Contact Animation
  if (mq.matches) {
    $(window).bind('scroll', function() {
      var navHeight = $(window).height() + 3200;
      if ($(window).scrollTop() > navHeight) {
          $('.jsAnimation').addClass('containerOfcontaintainer');
      }  else {
          $('.jsAnimation').removeClass('containerOfcontaintainer');
      } 
  });
  } else {
    $(window).bind('scroll', function() {
      var navHeight = $(window).height() + 1600;
      if ($(window).scrollTop() > navHeight) {
          $('.jsAnimation').addClass('containerOfcontaintainer');
      }  else {
          $('.jsAnimation').removeClass('containerOfcontaintainer');
